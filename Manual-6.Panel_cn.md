...
#### 6.3.1 IPanelManager中的注册器IPanelRegiester和加载器IIPanelLoaderAdapter都功能自定义，只有实现相应接口即可。
  ![image](assets/img/panel_13.png)

  ![image](assets/img/panel_14.png)

#### 6.3.2 IPanelManger中的SetShowMoment可以设置面板展示时机(立即展示|帧结束展示)
...
#### 6.3.3 IPanelSettings为面板注册时保存的面板配置信息接口，包含资源配置、背景配置和动画配置。
